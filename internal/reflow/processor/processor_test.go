...
func TestBuiltinProcessFlowBuildsPseudoPacketModelFromTuple(t *testing.T) {
	proc := NewBuiltin(config.ProcessorConfig{
		Builtin: config.BuiltinProcessorConfig{
	...
	fields := events[0].Fields
	headerData, ok := fields["header_data"].([]byte)
	if !ok || len(headerData) == 0 {
		t.Fatalf("expected pseudo packet header_data to be built, got %T len=%d", fields["header_data"], len(headerData))
	}
	if events[0].Packet == nil {
		t.Fatalf("expected packet model to be attached")
	}
	if got := fields["packet_ip_depth"]; got != uint32(1) {
		t.Fatalf("expected packet_ip_depth=1, got %#v", got)
	}
	if got := fields["src_addr"]; got != "192.0.2.1" {
		t.Fatalf("expected src_addr=192.0.2.1, got %#v", got)
	}
...
func TestBuiltinProcessFlowBuildsEncapsulatedPseudoPacket(t *testing.T) {
	proc := NewBuiltin(config.ProcessorConfig{
		Builtin: config.BuiltinProcessorConfig{
			BuildPseudoPacket: true,
		},
	...
	headerData, ok := fields["header_data"].([]byte)
	if !ok || len(headerData) == 0 {
		t.Fatalf("expected encapsulated pseudo packet header_data, got %T len=%d", fields["header_data"], len(headerData))
	}
	if events[0].Packet == nil {
		t.Fatalf("expected packet model to be attached")
	}
	...
	expectedLayers := []string{"ethernet", "ipv4", "gre", "ipv4", "tcp"}
