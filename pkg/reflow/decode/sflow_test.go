...
func TestDecodeSFlowCounterPacketExampleRejectsMalformedSampleLengths(t *testing.T) {
	...
	_, err = (&builtIn{}).decodeSFlow(&event.Event{
		Source:  event.SourceMetadata{Type: "flow"},
		Payload: append([]byte(nil), payload...),
	}, payload, 5)
	if err == nil {
		t.Fatalf("expected malformed packet to return a decode error")
	}
	if got := err.Error(); !strings.Contains(got, "sample 1 length") {
		t.Fatalf("expected sample length error, got %v", err)
	}
}

func TestDecodeSFlowExpandedCounterPacketExample(t *testing.T) {
	payload, err := hex.DecodeString("")
	if err != nil {
		t.Fatalf("DecodeString returned error: %v", err)
	}
	...
	for i, evt := range decoded {
		fields := evt.Fields
		if got := fields["flow_type"]; got != "sflow" {
			t.Fatalf("event %d expected flow_type=sflow, got %#v fields=%#v", i, got, fields)
		}
		if got := fields["record_kind"]; got != "interface_counter" {
			t.Fatalf("event %d expected record_kind=interface_counter, got %#v fields=%#v", i, got, fields)
		}
		if got := fields["counter_format"]; got != "expanded" {
			t.Fatalf("event %d expected counter_format=expanded, got %#v fields=%#v", i, got, fields)
		}
	}
}
