...
`), 0o644); err != nil {
	...
	interfaceName := cfg.Encoder.TemplatedFlow.Data.Catalog["interface_name"]
	if interfaceName.ID != 82 || interfaceName.Type != "string" {
		t.Fatalf("expected embedded interface_name interfaceName field, got %#v", interfaceName)
	}
