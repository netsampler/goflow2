...
// New constructs an HTTP server with metrics, health, and templates endpoints.
func New(cfg Config, templates TemplateSource, isCollecting func() bool) *http.Server {
	mux := http.NewServeMux()
	mux.Handle("/metrics", promhttp.Handler())
	mux.HandleFunc("/__health", func(wr http.ResponseWriter, r *http.Request) {
		if !isCollecting() {
	...
	if cfg.TemplatePath != "" && templates != nil {
		mux.HandleFunc(cfg.TemplatePath, func(wr http.ResponseWriter, r *http.Request) {
			values := templates()
			if values == nil {
