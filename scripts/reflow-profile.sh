...
case "$preset" in
	processor-json)
		run_profile processor-json ./pkg/reflow/processor 'BenchmarkBuiltinProcess/(goflow2v2_json|reflow_json)'
		;;
	encode)
		run_profile encode ./pkg/reflow/encode 'BenchmarkEncode'
		;;
	pipeline)
		run_profile pipeline ./pkg/reflow/app 'BenchmarkRunPipeline'
		;;
	aggregate)
		run_profile aggregate ./pkg/reflow/aggregate 'BenchmarkStateful(Process|Close)'
		;;
	decode)
		run_profile decode ./pkg/reflow/decode 'BenchmarkDecode'
		;;
	all)
		run_profile processor-json ./pkg/reflow/processor 'BenchmarkBuiltinProcess/(goflow2v2_json|reflow_json)'
		run_profile encode ./pkg/reflow/encode 'BenchmarkEncode'
