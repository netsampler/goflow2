...
func TestDecodeNetFlowV9IgnoresZeroPaddedFlowSetHeader(t *testing.T) {
	store := newTestTemplateStore()
	ctx := FlowContext{RouterKey: "router1"}
	...
	// A full zero-filled FlowSetHeader{Id: 0, Length: 0} worth of trailing
	// padding -- some exporters (observed: Cisco ACI/APIC) zero-pad NetFlow
	// v9 datagrams by 4+ bytes, which previously decoded as a bogus FlowSet
	// and failed with "negative length".
